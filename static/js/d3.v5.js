...
function create$1(node, id, self) {
  ...
  function start(elapsed) {
    ...
    for (i in schedules) {
      ...
      if (o.state === RUNNING) {
        o.state = ENDED;
        o.timer.stop();
        o.on.call("interrupt", node, node.__data__, o.conn, o.group);
        delete schedules[i];
      }
      ...
      // Cancel any pre-empted transitions.
        ...
        o.state = ENDED;
        o.timer.stop();
        o.on.call("cancel", node, node.__data__, o.conn, o.group);
        delete schedules[i];
      }
    }
    ...
    self.state = STARTING;
    self.on.call("start", node, node.__data__, self.conn, self.group);
    ...
    for (i = 0, j = -1; i < n; ++i) {
      if (o = self.tween[i].value.call(node, node.__data__, self.conn, self.group)) {
        tween[++j] = o;
      }
    }
    tween.length = j + 1;
  ...
  function tick(elapsed) {
    ...
    if (self.state === ENDING) {
      self.on.call("end", node, node.__data__, self.conn, self.group);
      stop();
    }
  }
...
function interrupt(node, name) {
  ...
  for (i in schedules) {
    ...
    schedule$$1.on.call(active ? "interrupt" : "cancel", node, node.__data__, schedule$$1.conn, schedule$$1.group);
    delete schedules[i];
  }
...
function collide(radius) {
  ...
  function prepare(quad) {
    if (quad.data) return quad.r = radii[quad.data.conn];
  ...
  function initialize() {
    ...
    radii = new Array(n);
    for (i = 0; i < n; ++i) node = nodes[i], radii[node.conn] = +radius(node, i, nodes);
  }
...
function link(links) {
  ...
  function defaultStrength(link) {
    return 1 / Math.min(count[link.source.conn], count[link.target.conn]);
  }
  ...
  function initialize() {
    ...
    for (i = 0, count = new Array(n); i < m; ++i) {
      ...
      count[link.source.conn] = (count[link.source.conn] || 0) + 1;
      count[link.target.conn] = (count[link.target.conn] || 0) + 1;
    }

    for (i = 0, bias = new Array(m); i < m; ++i) {
      link = links[i], bias[i] = count[link.source.conn] / (count[link.source.conn] + count[link.target.conn]);
    }
...
function manyBody() {
  ...
  function initialize() {
    ...
    for (i = 0; i < n; ++i) node = nodes[i], strengths[node.conn] = +strength(node, i, nodes);
  }
  ...
  function accumulate(quad) {
    ...
    // For leaf nodes, accumulate forces from coincident quadrants.
      ...
      q.x = q.data.x;
      q.y = q.data.y;
      do strength += strengths[q.data.conn];
      while (q = q.next);
    }
  ...
  function apply(quad, x1, _, x2) {
    ...
    do if (quad.data !== node) {
      w = strengths[quad.data.conn] * alpha / l;
      node.vx += x * w;
      node.vy += y * w;
...
function createEdge(left, right, v0, v1) {
  ...
  cells[left.conn].halfedges.push(index);
  cells[right.conn].halfedges.push(index);
  return edge;
}
  ...
  triangles: function() {
    ...
    this.cells.forEach(function(cell, i) {
      ...
      while (++j < m) {
        ...
        s1 = e1.left === site ? e1.right : e1.left;
        if (s0 && s1 && i < s0.conn && i < s1.conn && triangleArea(site, s0, s1) < 0) {
